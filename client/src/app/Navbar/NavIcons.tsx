...
const NavIcons = ({
  ...
  return (
    <li className="relative text-primary">
        {  MenuIcon ?  <MenuIcon onClick={toggle} 
        ...
        <img 
            onClick={toggle}
            src={`${BASE_API}${currentUser?.profilePicture}`} 
        ...
        {isMenuOpen && 
        <> 
           <div className={`absolute ${pointerPos} size-4 bg-black
            transform rotate-45 rounded`}/>
            {/* Menu Body */}
            <div className={`absolute p-2 w-[170px] text-sm rounded bg-black ${bodyPos}`}>
