...
const useAppointmentForm = ({
  ...
  const selectedService = useMemo(() => {
    return appointmentForm.service_id
      ? services?.find(
          (serve) => serve.service_id === appointmentForm.service_id
        )?.name
      : undefined;
  }, [appointmentForm.service_id, services]);

  const servicePrice = services
    ? services.find((serve) => serve.service_id === appointmentForm.service_id)
        ?.price
    : undefined;
  ...
  const { data: consultants, isLoading: isConsultantsLoading } =
    useGetConsultantsQuery(selectedService || undefined, {
      skip: !selectedService,
    });

  const serviceOptions =
    services?.map((service: GetServiceResponse) => ({
      value: service.service_id,
      label: service.name,
    })) || [];

  const consultantOptions =
    consultants?.map((consultant: GetConsultantsResponse) => ({
      value: consultant.consultant_id,
      label: consultant.name,
    })) || [];

  const dispatch = useAppDispatch();
  const handleCreateAppointment = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    dispatch(openModal("confirmAppointment"));
  };
