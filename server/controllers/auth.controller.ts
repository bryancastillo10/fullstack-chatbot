...
export const userSignUp = async (
  ...
  try {
    ...
    if (!usernameRegex.test(username)) {
      res.status(400).json({
        error: "Username must be greater than 5 alphanumeric characters.",
      });
      return;
    }
    ...
    const newUser = await prisma.user.create({
      data: {
        ...
        profilePicture: "/default/defaultprofilepic.png",
      },
    });
    ...
    if (newUser) {
      ...
      res
        .status(201)
        .json({ message: "User sign up is successful", user: newUser });
    }
  } catch (error) {
...
export const userSignIn = async (
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "User just logged in successfully", user: authUser });
  } catch (error) {
